...
class ResumeAnalyzer:
    ...
    def calculate_tfidf_match(self, resume_text, job_description):
        """Calculate TF-IDF based cosine similarity between resume and job description"""
        if not resume_text or not job_description:
            return {'tfidf_score': 0.0, 'cosine_similarity': 0.0}
        ...
        try:
            ...
            print(f"Error calculating TF-IDF match: {str(e)}")
            return {'tfidf_score': 0.0, 'cosine_similarity': 0.0}
    ...
    def analyze_resume(self, resume_data, job_requirements):
        ...
        try:
            ...
            # Calculate TF-IDF cosine similarity with job description
            job_description = job_requirements.get('job_description', '')
            tfidf_match = self.calculate_tfidf_match(text, job_description)
            ...
            # Blend keyword score with TF-IDF score for final skills match
            tfidf_score = tfidf_match.get('tfidf_score', 0)
            if tfidf_score > 0:
                skills_score = (skills_score * 0.5) + (tfidf_score * 0.5)
